...
export function createPeel(
  ...
  function compile(type: number, text: string): WebGLShader {
    ...
    if (!gl!.getShaderParameter(shader, gl!.COMPILE_STATUS)) {
      // Upstream logs and carries on with a broken program. Returning null
      // instead routes into the same fallback the component already has for a
      // missing context, so the sheet degrades rather than rendering blank.
      throw new Error(`Peel shader failed: ${gl!.getShaderInfoLog(shader)}`);
    }
    return shader;
  }
  ...
  function link(vertText: string, fragText: string) {
    ...
    // Upstream indexes a Record here. This repo compiles with
    // noUncheckedIndexedAccess, so that yields `| undefined` and every
    // gl.uniform* call rejects it. A Map read through a helper gives `| null`,
    // which those calls already accept as "no such uniform".
    ...
    const u = (name: string) => uniforms.get(name) ?? null;
    return { program, vert, frag, u };
  }

  let sheet: ReturnType<typeof link>;
  try {
    sheet = link(SHEET_VERT, SHEET_FRAG);
  } catch {
    return null;
  }
  ...
  capture = () => {
    ...
    try {
      ...
      // A capture can fail mid-layout; the next paint retries.
    }
  };
    ...
    if (probeCtx) {
      ...
      while (el) {
        ...
        if (bg && bg !== "transparent") {
          ...
          // Same reason: destructuring the pixel array gives `number | undefined`.
  ...
  function render() {
    ...
    gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
    gl!.uniform1i(sheet.u("uContent"), 0);
    gl!.uniform2f(sheet.u("uRes"), w, h);
    gl!.uniform1f(sheet.u("uSide"), side);
    gl!.uniform1f(sheet.u("uPeel"), peel.a);
    gl!.uniform1f(sheet.u("uReveal"), Math.max(config.reveal, 0));
    gl!.uniform1f(sheet.u("uCurl"), Math.max(config.curl, 1));
    gl!.uniform1f(sheet.u("uBow"), config.bow);
    gl!.uniform1f(sheet.u("uFocal"), Math.max(config.perspective, 200));
    gl!.uniform1f(sheet.u("uShade"), config.shade);
    gl!.uniform1f(sheet.u("uZone"), Math.max(config.zone, 1));
    gl!.uniform1f(sheet.u("uBulge"), Math.max(config.bulge, 0));
    gl!.uniform1f(sheet.u("uShine"), Math.max(config.shine, 0));
    gl!.uniform3f(
      sheet.u("uShineColor"),
      shineRgb[0],
      shineRgb[1],
    ...
    gl!.uniform1f(sheet.u("uCross"), side < 1.5 ? h : w);
    gl!.uniform1f(
      sheet.u("uSpan"),
      config.shineDistance > 0 ? config.shineDistance : side < 1.5 ? w : h,
    );
    gl!.uniform2f(sheet.u("uPointer"), pointer.su, pointer.sv);
    gl!.uniform1f(sheet.u("uMaxX"), contentMaxX);
