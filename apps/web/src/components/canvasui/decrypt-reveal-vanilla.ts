...
function glyphShapes(
  ...
  for (let g = 0; g < count; g++) {
    ...
    for (let c = 0; c < 6; c++) {
      const circle = INNER_CIRCLES[c];
      if (!circle) continue;
      const cx = circle[0] * cellW;
      const cy = circle[1] * cellH;
      let sum = 0;
      let total = 0;
      for (let y = Math.floor(cy - radius); y <= Math.ceil(cy + radius); y++) {
        for (
          ...
          if (
            ...
            continue;
          const pxIdx = ((originY + y) * image.width + originX + x) * 4 + 3;
          sum += image.data[pxIdx] ?? 0;
        }
      }
  ...
  for (let c = 0; c < 6; c++) {
    ...
    if (peak > 0) {
      for (let g = 0; g < count; g++) { vectors[g * 6 + c] = (vectors[g * 6 + c] ?? 0) / peak; }
    }
  }
  return vectors;
}
...
export function createDecryptReveal(
  ...
  function link(frag: string) {
    ...
    const getU = (name: string): WebGLUniformLocation | null => uniforms[name] ?? null;
    return { program, vs, fs, uniforms, getU };
  }
  ...
  function renderCells() {
    ...
    const dpr = output.width / Math.max(output.clientWidth, 1);
    const [cw, ch] = cellSizePx(dpr);
    const getU = cellPass.getU;
    gl!.useProgram(cellPass.program);
    gl!.activeTexture(gl!.TEXTURE0);
    gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
    gl!.uniform1i(getU("uContent"), 0);
    gl!.activeTexture(gl!.TEXTURE1);
    gl!.bindTexture(gl!.TEXTURE_2D, shapeTexture);
    gl!.uniform1i(getU("uShapes"), 1);
    gl!.uniform2f(getU("uContentRes"), output.width, output.height);
    gl!.uniform2f(getU("uCellPx"), cw, ch);
    gl!.uniform1i(getU("uGlyphCount"), glyphCount);
    gl!.uniform1f(getU("uContrast"), Math.min(Math.max(config.contrast, 0.3), 3));
    gl!.uniform1f(getU("uExposure"), Math.min(Math.max(config.exposure, 0.2), 3));
    gl!.uniform1f(getU("uThreshold"), Math.max(config.threshold, 0.005));
    gl!.uniform3f(getU("uBg"), bg[0] ?? 0, bg[1] ?? 0, bg[2] ?? 0);
    gl!.bindFramebuffer(gl!.FRAMEBUFFER, cellFbo);
  ...
  function render() {
    ...
    gl!.activeTexture(gl!.TEXTURE1);
    gl!.bindTexture(gl!.TEXTURE_2D, cellTexture);
    gl!.uniform1i(getU("uCells"), 1);
    gl!.activeTexture(gl!.TEXTURE2);
    gl!.bindTexture(gl!.TEXTURE_2D, atlasTexture);
    gl!.uniform1i(getU("uAtlas"), 2);
    gl!.uniform2f(getU("uRes"), w, h);
    gl!.uniform1f(getU("uDpr"), dpr);
    gl!.uniform2f(getU("uCellPx"), cw, ch);
    gl!.uniform2f(getU("uGrid"), cellCols, cellRows);
    gl!.uniform2f(getU("uAtlasGrid"), atlasCols, atlasRows);
    gl!.uniform2f(getU("uAtlasPad"), atlasPad[0] ?? 0, atlasPad[1] ?? 0);
    gl!.uniform2f(getU("uAtlasInner"), atlasInner[0] ?? 0, atlasInner[1] ?? 0);
    gl!.uniform1i(getU("uGlyphCount"), glyphCount);
    gl!.uniform2f(getU("uPointer"), pointer.x, pointer.y);
    gl!.uniform1f(getU("uActive"), pointer.active);
    gl!.uniform1f(getU("uRadius"), Math.max(config.radius, 1));
    gl!.uniform1f(getU("uSoftness"), config.softness);
    gl!.uniform1f(getU("uColored"), config.colored);
    gl!.uniform3f(getU("uColor"), fg[0] ?? 0, fg[1] ?? 0, fg[2] ?? 0);
    gl!.uniform1f(getU("uBrightness"), Math.min(Math.max(config.brightness, 0.2), 3));
    gl!.uniform1f(getU("uLegibility"), Math.min(Math.max(config.legibility, 0), 1));
    gl!.uniform1f(getU("uScramble"), Math.min(Math.max(config.scramble, 0), 1));
    gl!.uniform1f(
      getU("uScrambleSpeed"),
      Math.min(Math.max(config.scrambleSpeed, 0), 30),
    );
    gl!.uniform1f(getU("uEdgeWidth"), config.edgeWidth);
    gl!.uniform1f(getU("uEdgeFlicker"), Math.min(Math.max(config.edgeFlicker, 0), 1));
    gl!.uniform1f(getU("uEdgeGlow"), Math.min(Math.max(config.edgeGlow, 0), 3));
    gl!.uniform1f(getU("uEdgeTint"), config.edgeTint);
    gl!.uniform1f(getU("uAberration"), Math.max(config.aberration, 0));
    gl!.uniform1f(getU("uPassthrough"), config.passthrough);
    gl!.uniform3f(getU("uBg"), bg[0] ?? 0, bg[1] ?? 0, bg[2] ?? 0);
    gl!.uniform1f(getU("uTime"), time);
    gl!.uniform1f(getU("uMaxX"), contentMaxX);
    gl!.uniform1f(getU("uCrisp"), reducedMotion || !htmlInCanvas ? 1 : 0);
    gl!.bindFramebuffer(gl!.FRAMEBUFFER, null);
