...
export const neutralTheme = defineTheme({
  ...
  //
  // The family is a variable, not the literal name: `astryx theme build` warns
  // that a theme naming a font it does not load leaves every browser falling
  // back in silence. next/font fills --font-figtree with a self-hosted family,
  // so the font ships with the app instead of costing a render-blocking
  // request to a third party on first paint.
